...
void ObjectMgr::LoadCreatureOutfits()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("server.loading", ">> Outfit entry %u in `creature_template_outfits` has too low entry (entry <= %u). Ignoring.", entry, CreatureOutfit::max_real_modelid);
            continue;
        }
        ...
        {
            LOG_ERROR("server.loading", ">> Outfit entry %u in `creature_template_outfits` has incorrect npcsoundsid (%u). Using 0.", entry, co->npcsoundsid);
            co->npcsoundsid = 0;
        }
        ...
        if (!rEntry)
        {
            LOG_ERROR("server.loading", ">> Outfit entry %u in `creature_template_outfits` has incorrect race (%u).", entry, uint32(co->race));
            continue;
        }
        ...
        if (!cEntry)
        {
            LOG_ERROR("server.loading", ">> Outfit entry %u in `creature_template_outfits` has incorrect class (%u).", entry, uint32(co->Class));
            continue;
        }
        ...
        default:
            LOG_ERROR("server.loading", ">> Outfit entry %u in `creature_template_outfits` has invalid gender %u", entry, uint32(co->gender));
            continue;
        }
        ...
        for (EquipmentSlots slot : CreatureOutfit::item_slots)
        {
            ...
            if (displayInfo > 0) // entry
            {
                ...
                {
                    LOG_ERROR("server.loading", ">> Outfit entry %u in `creature_template_outfits` has invalid item entry: %u. Ignoring.", entry, item_entry);
    ...
    LOG_INFO("server.loading", ">> Loaded %u creature outfits in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
